...
The tutorial dataset was collected on an Electra Neuromag MEG scanner and use the Neuromag coordinate system. The Neuromag coordinate system is defined form external landmarks -- i.e. the fiducials, we defined as the first step when we digitized the subject. The coordinate system is defined like this:
...
Now we will load the headpoint from the MEG/EEG data with _ft_read_headshape_. This read the head points from the file. 
```{r, engine='octave', eval=FALSE}
...
The head point is in the Neuromag coordinate system. Toghether with the location of the head relative to the MEG sensors and the location of the EEG electrodes we will now how to "fit" the anatomical data. Let us load the MEG sensor and EEG electrode locations and inspect:

```{r, engine='octave', eval=FALSE}
grad    = ft_read_sens(input_path,'senstype','meg'); % Load MEG sensors
elec    = ft_read_sens(input_path,'senstype','eeg'); % Load EEF electrodes
...
* Q: What could be a potential problem with how we define the conductivity of the different compartments, and how does the problem affect the source reconstruction of MEG and EEG respectively?
...
ft_plot_vol(headmodel_meg, 'unit', 'cm')
...
```{r, engine='octave', eval=FALSE}
figure
ft_plot_sens(elec, 'unit', 'cm')
ft_plot_headshape(headshape, 'unit', 'cm')
ft_plot_vol(headmodel_eeg, 'unit', 'cm','facealpha', 0.5, 'edgecolor','r')
ft_plot_axes([], 'unit', '´cm');
view(132, 14)
